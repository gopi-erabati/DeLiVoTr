...
class DynamicVFECustom(BaseModule):
    ...
    def __init__(self,
        ...
        super(DynamicVFECustom, self).__init__(init_cfg=init_cfg)
        ...
        self.scatter = DynamicScatterCustom(voxel_size, point_cloud_range, True)
        ...
        self.vfe_scatter = DynamicScatterCustom(voxel_size, point_cloud_range,
                                          (mode != 'max'))
        self.cluster_scatter = DynamicScatterCustom(
    ...
    def map_voxel_center_to_point(self, pts_coors, voxel_mean, voxel_coors):
        ...
        pts_coors[pts_coors == -1] = 0
        # sometimes the pts_cors during voxelization is assigned to -1 which
        # gives index error in canvas[voxel_index.long()]
        voxel_index = (
                pts_coors[:, 0] * canvas_z * canvas_y * canvas_x +
                pts_coors[:, 1] * canvas_y * canvas_x +
                pts_coors[:, 2] * canvas_x + pts_coors[:, 3])
        assert voxel_index.max().item() < canvas_len, f'voxel index ' \
                                                f'{voxel_index.max().item()} ' \
                                                      f'is out of bounds for indices upto {canvas_len}'
