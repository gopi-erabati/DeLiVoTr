# Adapted from modules of fairseq
...
class SingleHeadAttention(nn.Module):
    ...
    def __init__(self, q_in_dim, k_in_dim, v_in_dim, proj_dim, out_dim,
        ...
        self.k_embed_dim = k_in_dim
        self.v_embed_dim = v_in_dim
        ...
        self.linear_k = get_weight_layer(name='linear',
                                         in_features=self.k_embed_dim,
                                         out_features=self.proj_dim,
                                         use_bias=True,
                                         gates=1
                                         )
        self.linear_v = get_weight_layer(name='linear',
                                         in_features=self.v_embed_dim,
                                         out_features=self.proj_dim,
                                         use_bias=True,
                                         gates=1
                                         )
    ...
    def forward(
            ...
            incremental_state: Optional[
                Dict[str, Dict[str, Optional[Tensor]]]] = None,
        ...
        tgt_len, bsz, q_embed_dim = query.size()  # (N, B, d0)
        assert q_embed_dim == self.q_embed_dim, 'Error in {}. {} != {}'.format(
            self.__class__.__name__, q_embed_dim,
            self.q_embed_dim)
        ...
        q = q.contiguous().transpose(0, 1)  # (B, N, d0)

        if k is not None:
            k = k.contiguous().transpose(0, 1)  # (B, N, d0)

        if v is not None:
            v = v.contiguous().transpose(0, 1)  # (B, N, d0)
        ...
        # (B, N, d0) x (B, d0, N) --> (B, N, N)
        attn_weights = torch.bmm(q, k.transpose(1, 2))

        attn_weights = SingleHeadAttention.apply_sparse_mask(attn_weights,
                                                             tgt_len, src_len,
                                                             bsz)
        ...
        if need_weights:
            ...
            return attn, attn_weights_tmp  # (N, B, d)
