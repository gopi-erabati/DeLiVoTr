...
from .helpers import print_warning_message
...
class DExTraUnit(nn.Module):
    ...
    def __init__(self,
        ...
        self.glt_shuffle = glt_shuffle  # no shuffling in ICLR version
    ...
    @staticmethod
    def dextra_config(in_features, out_features, max_features, n_layers,
                      max_groups):

        mid_point = int(math.ceil(n_layers / 2.0))
        # decide number of groups per layer
        groups_per_layer = [min(2 ** (i + 1), max_groups) for i in
                            range(mid_point)]

        # divide the space linearly between input_features and max_features
        output_sizes = np.linspace(in_features, max_features, mid_point,
                                   dtype=np.int).tolist()
        ...
        assert len(output_sizes) == len(groups_per_layer), '{} != {}'.format(
            len(output_sizes), len(groups_per_layer))
        ...
        for i in range(n_layers - 1):
            ...
            if out_dim_l % 8 != 0:
                print_warning_message(
                    'To maximize tensor usage, output dimension {} should be divisible by 8'.format(
                        out_dim_l))

            if inp_dim_l1 % 8 != 0:
                print_warning_message(
                    'To maximize tensor usage, input dimension {} should be divisible by 8'.format(
                        inp_dim_l1))
    ...
    @staticmethod
    def define_config(in_features, out_features, max_features, n_layers,
                      max_groups):
        # decide number of groups per layer
        groups_per_layer = []
        counter = 0
    ...
    def forward_dextra(self, x):
        '''
        :param x: Input is (B, N, d0)
        :return: output is (B, N, d0)
        '''
        B = x.size(0)
        T = x.size(1)
        ...
        for i, layer_i in enumerate(self.dextra_layers):
            ...
            g_next_layer = self.groups_per_layer[
                i + 1] if i < self.num_glt_layers - 1 else 1
            if g_next_layer == 1:
    ...
    def forward(self, x):
        '''
        :param x: Input is (B, N, d)
        :return: Output is (B, N, d0)
        '''

        # process input
        x = self.input_layer(x)  # (B, N, d0)
        n_dims = x.dim()
