...
from .helpers import get_norm_layer, get_activation_layer, print_error_message
...
class GroupLinear(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, n_groups: int = 4,
                 use_bias: bool = False,
                 use_shuffle: bool = False,
                 norm_type: Optional[str] = None, dropout: float = 0.0,
        ...
        if in_features % n_groups != 0:
            err_msg = "Input dimensions ({}) must be divisible by n_groups (" \
                      "{})".format(
                in_features, n_groups)
            print_error_message(err_msg)
        if out_features % n_groups != 0:
            err_msg = "Output dimensions ({}) must be divisible by n_groups " \
                      "({})".format(
        ...
        if norm_type is not None:
            self.normalization_fn = get_norm_layer(name=norm_type,
                                                   out_features=out_groups)
    ...
    def compute_macs_params(self):
        ...
        if self.normalization_fn is not None:
            n_params += sum(
                [p.numel() for p in self.normalization_fn.parameters()])
...
class Linear(nn.Module):
    ...
    def __init__(self, in_features, out_features, use_bias=True, num_gates=1,
        ...
        self.weights = torch.nn.Parameter(
            torch.Tensor(out_features * num_gates, in_features))
        if use_bias:
            self.bias = torch.nn.Parameter(
                torch.Tensor(out_features * num_gates))
        else:
            self.bias = None

        if norm_type is not None:
            self.normalization_fn = get_norm_layer(name=norm_type,
                                                   out_features=out_features * num_gates)
            self.norm_type = norm_type
        else:
            self.normalization_fn = None
            self.norm_type = None
...
def get_weight_layer(name: str, in_features: int, out_features: int,
                     groups: int = 4, use_bias: bool = True,
    ...
    if name == 'linear':
        layer = Linear(in_features=in_features, out_features=out_features,
                       use_bias=use_bias, num_gates=gates,
                       norm_type=norm_type, dropout=dropout, act_type=act_type)
    elif name == 'glt':
        layer = GroupLinear(in_features=in_features, out_features=out_features,
                            n_groups=groups,
                            use_bias=use_bias, use_shuffle=shuffle,
                            norm_type=norm_type,
