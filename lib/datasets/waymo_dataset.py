...
from mmdet3d.core.bbox import Box3DMode, points_cam2img, Coord3DMode
...
class CustomWaymoDataset(KittiDataset):
    ...
    def format_results(self,
        ...
        if 'waymo' in data_format:
            from mmdet3d.core.evaluation.waymo_utils \
                .prediction_kitti_to_waymo import \
                KITTI2Waymo  # noqa
            waymo_root = osp.join(
    ...
    def evaluate(self,
        ...
        assert (
                'waymo' in metric or 'kitti' in metric or 'raw' in metric or 'fast' in metric), \
        ...
        if 'waymo' in metric:
            ...
            ret_bytes = subprocess.check_output(
                'lib/core/evaluation/waymo_utils/' +
        ...
        if 'fast' in metric:
            waymo_root = osp.join(self.data_root.split('kitti_format')[0],
                                  'waymo_format')
            self.fast_convert_to_waymo(results, pklfile_prefix)
            import subprocess
            ret_bytes = subprocess.check_output(
                '/home/gopi/PhD/code/objdet/open-mmlab/mmdetection3d/mmdet3d/core/evaluation/waymo_utils/' +
                f'compute_detection_metrics_main {pklfile_prefix}.bin ' +
                f'{waymo_root}/gt.bin',
                shell=True)
            ret_texts = ret_bytes.decode('utf-8')
    ...
    def prepare_train_data(self, index):
        ...
        example = input_dict
        for transform, transform_type in zip(self.pipeline.transforms,
                                             self.pipeline_types):
    ...
    def lidar2waymo_box(self, in_box, score, class_name, context_name,
                        timestamp):
    ...
    def show(self, results, out_dir, show=True, pipeline=None):
        ...
        for i, result in tqdm(enumerate(results)):
            ...
            #              2600, 2726, 2981]:
            #     continue
            if i not in [2559, 2599, 2605, 2993, 3000, 4716, 5613, 5845, 6167,
                         6333, 8756, 11139]:
                continue
